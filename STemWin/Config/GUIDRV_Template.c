...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    MD070SD_PutPixel(x, y, PixelIndex);     //调用液晶屏打点函数
}
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  ...
  PixelIndex = MD070SD_GetPixel(x, y);     //调用液晶屏读点函数  
  return PixelIndex;
}
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
    ...
    ColorIndex = GUI_GetColorIndex();
    MD070SD_Draw_Rectangle(x0, y0, x1, y1, ColorIndex);
}
...
static void _DrawHLine(GUI_DEVICE * pDevice, int x0, int y, int x1) {
  //_FillRect(pDevice, x0, y, x1, y);  //调用填充矩形函数（直线是特殊的矩形）
    
    //调用液晶屏画线函数否则无法画线
    LCD_PIXELINDEX ColorIndex;
    ColorIndex = GUI_GetColorIndex();
    MD070SD_Draw_Line(x0, y, x1, y,ColorIndex);
}
...
static void _DrawVLine(GUI_DEVICE * pDevice, int x, int y0, int y1) {
  //_FillRect(pDevice, x, y0, x, y1);
    ...
    LCD_PIXELINDEX ColorIndex;
    ColorIndex = GUI_GetColorIndex();
    MD070SD_Draw_Line(x, y0, x, y1,ColorIndex);
}
...
static void _DrawBitLine16BPP(GUI_DEVICE * pDevice, int x, int y, U16 const GUI_UNI_PTR * p, int xsize) {
  ...
  for (;xsize > 0; xsize--, x++, p++) {
   ...
   //设置列起始坐标
	...
	MD070SD_Write_Data(x);
    //设置列终止坐标
    MD070SD_Write_Cmd(END_COL_REG);
	MD070SD_Write_Data(x+xsize);
	
	//设置行起始坐标
	MD070SD_Write_Cmd(START_ROW_REG);
	MD070SD_Write_Data(y);
    //设置行终止坐标
